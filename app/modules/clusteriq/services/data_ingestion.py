...
Pulls keyword/URL performance data from two sources:
  ...
  IngestionOrchestrator      — ties both together for a full project ingestion run
...
class IngestionOrchestrator:
    """
    Coordinates a full ClusterIQ data-ingestion run:
      1. GSC URL × keyword matrix
      2. DataForSEO SERP for the top-N keywords that lack serp_urls
    """

    def __init__(
        self,
        gsc_service:      GSCIngestionService | None = None,
        dataforseo_service: DataForSEOIngestionService | None = None,
    ) -> None:
        self._gsc = gsc_service or GSCIngestionService()
        # DataForSEO service is optional — skip SERP fetch if credentials missing
        if dataforseo_service is not None:
            self._dfs: DataForSEOIngestionService | None = dataforseo_service
        else:
            try:
                self._dfs = DataForSEOIngestionService()
            except ValueError:
                logger.warning(
                    "[Orchestrator] DataForSEO credentials not configured — SERP fetch disabled"
                )
                self._dfs = None
    ...
    async def run_full_ingestion(
        self,
        project_id: int,
        top_keywords_limit: int = 500,
    ) -> dict[str, Any]:
        """
        Execute a full ingestion run for a ClusterIQ project.

        Steps:
          1. Load project and validate it has a gsc_property set.
          2. Set project status → 'crawling'.
          3. Fetch GSC URL × keyword matrix (all impressions > 0).
          4. Pick the top-N keywords by total impressions.
          5. Filter to keywords that have no serp_urls yet.
          6. Fetch SERP data from DataForSEO for those keywords.
          7. Set project status → 'clustering' (ready for graph phase).
          8. Return summary dict.

        Returns:
            {
              "urls_found":     int,   # distinct URLs seen in GSC
              "keywords_found": int,   # distinct keywords seen in GSC
              "serp_fetched":   int,   # keywords with SERP data fetched this run
              "credits_used":   int,   # DataForSEO tasks consumed
              "cost_usd":       float, # estimated USD spent on DataForSEO
            }
        """
        ...
        if not project.gsc_property:
            raise ValueError(f"CluProject id={project_id} has no gsc_property set")

        await self._set_status(project_id, "crawling")
        logger.info(
            "[Orchestrator] project=%d (%s) — ingestion started", project_id, project.domain
        )

        # --- Step 1: GSC matrix -----------------------------------------
        try:
            gsc_rows = await self._gsc.fetch_url_keyword_matrix(
                project_id, project.gsc_property
            )
        except GSCAuthError as exc:
            await self._set_status(project_id, "error")
            raise

        # Gather distinct counts for summary
        distinct_stats = await self._count_distinct(project_id)

        # --- Step 2: Select top keywords for SERP fetch -----------------
        if self._dfs is None:
            logger.info("[Orchestrator] Skipping DataForSEO — no credentials")
            serp_result = {"fetched": 0, "failed": 0, "credits_used": 0}
        else:
            top_keywords = await self._top_keywords_without_serp(
                project_id, limit=top_keywords_limit
            )
            logger.info(
                "[Orchestrator] project=%d — %d keywords selected for SERP fetch",
                project_id, len(top_keywords),
            )

            serp_result = await self._dfs.fetch_serp_for_keywords(
                project_id, top_keywords
            )

        # --- Step 3: Transition to clustering phase ---------------------
        await self._set_status(project_id, "clustering")

        cost_usd = (
            self._dfs.estimate_cost(serp_result["credits_used"]) if self._dfs else 0.0
        )

        summary = {
            "urls_found":     distinct_stats["urls"],
            "keywords_found": distinct_stats["keywords"],
            "serp_fetched":   serp_result["fetched"],
            "credits_used":   serp_result["credits_used"],
            "cost_usd":       cost_usd,
        }
        logger.info("[Orchestrator] project=%d — ingestion complete: %s", project_id, summary)
        return summary
    ...
    async def _count_distinct(self, project_id: int) -> dict[str, int]:
        ...
        async with AsyncSessionLocal() as db:
            ...
            kw_count = (await db.execute(
                select(func.count(func.distinct(CluSerpData.keyword)))
                .where(CluSerpData.project_id == project_id)
            )).scalar() or 0

        return {"urls": url_count, "keywords": kw_count}

    async def _top_keywords_without_serp(
        self,
        project_id: int,
        limit: int,
    ) -> list[str]:
        """
        Return the top-N keywords (by total impressions) that have no serp_urls yet.
        Uses a subquery to aggregate impressions across all URL rows for each keyword.
        """
        from sqlalchemy import func
        async with AsyncSessionLocal() as db:
            stmt = (
